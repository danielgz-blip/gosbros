export type Language = 'es' | 'en';

export const translations = {
  es: {
    // Navigation
    nav: {
      works: 'Proyectos',
      about: 'Nosotros',
      archive: 'Archivo',
      contact: 'Contacto',
      getInTouch: 'Contacto',
      menu: 'Menú',
      close: 'Cerrar',
      home: 'Inicio',
    },
    // Footer
    footer: {
      social: 'Social',
      links: 'Enlaces',
      location: 'Ubicación',
      privacyPolicy: 'Política de Privacidad',
      termsOfService: 'Términos de Servicio',
      city: 'Ciudad de México, MX',
    },
    // Home Page
    home: {
      locationTag: 'ESTÁS EN\nGOSBROS',
      heroTitle: 'BUENO, BONITO\NY A UN PRECIO\nJUSTO.',
      moreAboutUs: 'Más Sobre Nosotros',
      view: 'VER',
      missionTitle1: 'Trazamos nuestro\npropio camino.',
      missionTitle2: 'Diseño audaz, siempre en auge.',
      missionText: 'GOSBROS es un estudio híbrido con sede en la Ciudad de México, eliminando la brecha entre soluciones arquitectónicas de alta eficiencia e identidades visuales radicales. Diseño funcional pero hermoso, al alcance de todos.',
      strategy: 'Estrategia',
      architecture: 'Arquitectura',
      branding: 'Diseño Gráfico',
      scroll: 'Desplazar'
    },
    // Quote Calculator
    quote: {
      heading: 'COTIZA NUESTROS\nSERVICIOS',
      subheading: 'Transparencia radical. Sin sorpresas.',
      step: 'Paso',
      selectService: '¿Qué servicio necesitas?',
      architecture: 'Arquitectura',
      architectureDesc: 'Diseño arquitectónico, interiores y remodelación',
      branding: 'Diseño Gráfico',
      brandingDesc: 'Identidad de marca, logo y manual de marca',
      strategy: 'Estrategia',
      strategyDesc: 'Estrategia de marca, negocio y marketing',
      selectProjectType: 'Tipo de proyecto',
      selectArea: 'Área aproximada',
      customArea: 'Área personalizada (m²)',
      enterArea: 'Ingresa m²',
      selectScope: 'Alcance de entregables',
      selectClientSize: 'Tamaño de tu empresa (ingreso anual)',
      selectStrategyType: 'Tipo de estrategia',
      yourEstimate: 'Tu estimado',
      estimateDisclaimer: 'Este es un estimado preliminar. Los precios finales se definen en consulta personalizada.',
      whatsappCta: 'Cotizar por WhatsApp',
      emailCta: 'Solicitar Cotización',
      back: 'Atrás',
      next: 'Continuar',
      startOver: 'Empezar de nuevo',
      currency: 'Moneda',
    },
    // Works Page
    works: {
      title: 'PROYECTOS',
      view: 'VER'
    },
    // Archive Page
    archive: {
      title: 'ARCHIVO',
      filterAll: 'Todos',
      filterArchitecture: 'Arquitectura',
      filterBranding: 'Diseño Gráfico',
      colTitle: 'Título',
      colDiscipline: 'Disciplina',
      colMaterial: 'Material / Concepto',
      colEthos: 'Enfoque',
      colYear: 'Año'
    },
    // About Page
    about: {
      phrases: [
        "CONSTRUIMOS\nEL FUTURO.",
        "DISEÑAMOS\nCON IMPACTO.",
        "ALTA\nEFICIENCIA."
      ],
      capabilitiesTag: 'NUESTRAS\nCAPACIDADES',
      mainText: 'Somos un estudio híbrido especializado en Arquitectura, Diseño y Estrategia. \n\nCreemos que el diseño excepcional es un derecho, no un lujo. Inspirados en las paradas de autobús soviéticas — funcionales pero hermosas — democratizamos el acceso a una calidad implacable.',
      tags: 'Visualización 3D , Experiencia Espacial , Identidad de Marca , Naming , Arquitectura Comercial , Diseño de Interiores , Empaque , Editorial , Innovación de Producto , Urbanismo Modular .',
      valuesLabel: 'VALORES',
      valuesTitle: 'DEMOCRATIZANDO\nEL BUEN DISEÑO.',
      valuesList: [
        { title: 'Transparencia Radical', desc: 'Nuestros presupuestos son abiertos. Nuestras decisiones son lógicas. No hay misterio ni cargos ocultos.' },
        { title: 'Eficiencia Despiadada', desc: 'Eliminamos lo innecesario. Optimizamos recursos para ofrecer resultados de primera línea a costos accesibles.' },
        { title: 'Derecho Público', desc: 'El buen diseño no es un lujo. Es una utilidad pública. Diseñamos para elevar el estándar, no el precio.' }
      ]
    },
    // Contact Page
    contact: {
      title: 'EL BUEN DISEÑO\nA TU ALCANCE.',
      getInTouch: 'CONTACTO'
    },
    // Admin
    admin: {
      title: 'PANEL DE CONTROL',
      addProject: 'Añadir Proyecto',
      save: 'Guardar',
      success: 'Proyecto guardado con éxito.',
      error: 'Error al guardar.'
    }
  },
  en: {
    // Navigation
    nav: {
      works: 'Works',
      about: 'About',
      archive: 'Archive',
      contact: 'Contact',
      getInTouch: 'Get in touch',
      menu: 'Menu',
      close: 'Close',
      home: 'Home',
    },
    // Footer
    footer: {
      social: 'Social',
      links: 'Links',
      location: 'Location',
      privacyPolicy: 'Privacy Policy',
      termsOfService: 'Terms of Service',
      city: 'Mexico City, MX',
    },
    // Home Page
    home: {
      locationTag: 'YOU ARE IN\nGOSBROS',
      heroTitle: 'GOOD, BEAUTIFUL,\nAND FAIRLY\nPRICED.',
      moreAboutUs: 'More About Us',
      view: 'VIEW',
      missionTitle1: 'We pave our\nown way.',
      missionTitle2: 'Bold design, always in auge.',
      missionText: 'GOSBROS is a hybrid studio based in Mexico City, bridging the gap between high-efficiency architectural solutions and radical visual identities. Functional yet beautiful design, accessible to all.',
      strategy: 'Strategy',
      architecture: 'Architecture',
      branding: 'Branding',
      scroll: 'Scroll'
    },
    // Quote Calculator
    quote: {
      heading: 'CALCULATE\nYOUR QUOTE',
      subheading: 'Radical transparency. No surprises.',
      step: 'Step',
      selectService: 'What service do you need?',
      architecture: 'Architecture',
      architectureDesc: 'Architectural design, interiors and renovation',
      branding: 'Branding',
      brandingDesc: 'Brand identity, logo and brand manual',
      strategy: 'Strategy',
      strategyDesc: 'Brand, business and marketing strategy',
      selectProjectType: 'Project type',
      selectArea: 'Approximate area',
      customArea: 'Custom area (m²)',
      enterArea: 'Enter m²',
      selectScope: 'Scope of deliverables',
      selectClientSize: 'Company size (annual revenue)',
      selectStrategyType: 'Strategy type',
      yourEstimate: 'Your estimate',
      estimateDisclaimer: 'This is a preliminary estimate. Final pricing is defined in a personalized consultation.',
      whatsappCta: 'Quote via WhatsApp',
      emailCta: 'Request Quote',
      back: 'Back',
      next: 'Continue',
      startOver: 'Start over',
      currency: 'Currency',
    },
    // Works Page
    works: {
      title: 'WORKS',
      view: 'VIEW'
    },
    // Archive Page
    archive: {
      title: 'ARCHIVE',
      filterAll: 'All',
      filterArchitecture: 'Architecture',
      filterBranding: 'Branding',
      colTitle: 'Title',
      colDiscipline: 'Discipline',
      colMaterial: 'Primary Material',
      colEthos: 'Cost Ethos',
      colYear: 'Year'
    },
    // About Page
    about: {
      phrases: [
        "WE BUILD\nTHE FUTURE.",
        "WE DESIGN\nWITH IMPACT.",
        "HIGH\nEFFICIENCY."
      ],
      capabilitiesTag: 'OUR\nCAPABILITIES',
      mainText: 'We are a hybrid studio specializing in Architecture, Design, and Strategy. \n\nWe believe outstanding design is a right, not a luxury. Inspired by Soviet bus stops — functional yet beautiful — we democratize access to relentless quality.',
      tags: '3D Visualization , Spatial Experience , Brand Identity , Naming , Commercial Architecture , Interior Design , Packaging , Publishing , Product Innovation , Modular Urbanism .',
      valuesLabel: 'VALUES',
      valuesTitle: 'DEMOCRATIZING\nGOOD DESIGN.',
      valuesList: [
        { title: 'Radical Transparency', desc: 'Our budgets are open. Our decisions are logical. No mystery, no hidden agency fees.' },
        { title: 'Ruthless Efficiency', desc: 'We strip the unnecessary. We optimize resources to deliver top-tier results at accessible costs.' },
        { title: 'Public Right', desc: 'Good design is not a luxury. It is a public utility. We design to raise the standard, not the price.' }
      ]
    },
    // Contact Page
    contact: {
      title: 'GOOD DESIGN\nWITHIN YOUR\nREACH.',
      getInTouch: 'GET IN TOUCH'
    },
    // Admin
    admin: {
      title: 'ADMIN PANEL',
      addProject: 'Add Project',
      save: 'Save',
      success: 'Project saved successfully.',
      error: 'Error saving.'
    }
  }
};
